...
#ifdef GEKKO
		/* Check usb first, then sd card */
		/* Quick hack using escape sequence for ':' FIXME */
		if (File_DirExists("usb:/apps/xroar/"))
			path = find_in_path("usb:/apps/xroar/", filename);
		else if (File_DirExists("sd:/apps/xroar/"))
			path = find_in_path("sd:/apps/xroar/", filename);
#else
