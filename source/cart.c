...
#ifdef GEKKO
	int i;
	/* Always force cart type, not only at startup. */
	for(i=0; i<14; i++)
	{
		if (strcmp(cc->description, dragondos_roms[i]) == 0)
			cc->type = CART_DRAGONDOS;
	}

	for(i=0; i<2; i++)
	{
		if (strcmp(cc->description, rsdos_roms[i]) == 0)
			cc->type = CART_RSDOS;
	}
	for(i=0; i<3; i++)
	{
		if (strcmp(cc->description, deltados_roms[i]) == 0)
			cc->type = CART_DELTADOS;
	}
#endif
	switch (cc->type) {
		default:
