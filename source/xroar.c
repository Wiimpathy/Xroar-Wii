...
char *xroar_rgb = NULL;
int xroar_bilinear = 0;
int xroar_240p = 0;
int xroar_tvfilter = 0;
int xroar_wide = 0;
#endif
...
#ifdef GEKKO
	{ .value = CROSS_COLOUR_KBRW, .name = "blue-green", .description = "Blue-green (PAL-M)" },
	{ .value = CROSS_COLOUR_KRBW, .name = "green-blue", .description = "Green-blue (PAL-M)" },
	{ .value = CROSS_COLOUR_KRBW, .name = "User-defined", .description = "User defined" },
	{ .value = CROSS_COLOUR_KRBW, .name = "User-defined-invert", .description = "User defined (invert)" },
#endif
	XC_ENUM_END()
};
...
static struct xconfig_option xroar_options[] = {
	/* Emulated machine */
#ifdef GEKKO
	XC_OPT_CALL_1( "default-machine",  &set_machine ),
	XC_OPT_STRING( "rgb",              &xroar_rgb ),
	XC_OPT_BOOL  ( "bilinear",         &xroar_bilinear ),
	XC_OPT_BOOL  ( "240p",             &xroar_240p ),
	XC_OPT_INT   ( "tvfilter",         &xroar_tvfilter ),
	XC_OPT_INT   ( "wide",             &xroar_wide ),
#else
...
#define ROMPATH "/apps/xroar/"
#define CONFPATH "/apps/xroar/"
#else
#ifndef ROMPATH
# define ROMPATH "."
#endif
...
#ifdef GEKKO
	/* if rgb option found, add 2 colours */
	if(xroar_rgb == NULL)
	{
		//LOG_DEBUG(2, "xroar_rgb : %s. \n", xroar_rgb);
		#undef NUM_CROSS_COLOUR_PHASES
		#define NUM_CROSS_COLOUR_PHASES (5)
	}
	else
	{
		#undef NUM_CROSS_COLOUR_PHASES
		#define NUM_CROSS_COLOUR_PHASES (7)
	}
#endif
	switch (action) {
	case XROAR_CYCLE:
...
void xroar_set_machine(int machine_type) {
	...
	if (new >= 0 && new < num_machine_types) {
		...
		xroar_vdg_palette = get_machine_palette();
#ifdef GEKKO
		xroar_opt_tape_fast = TAPE_FAST;
		tape_set_state(TAPE_FAST);
#endif
